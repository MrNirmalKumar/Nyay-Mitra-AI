        ...
        <div className="flex flex-wrap gap-2">
          {SAMPLE_DOCS.map((sample, idx) => (
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-800 hover:bg-slate-700 text-gold-400 border border-gold-500/20 hover:border-gold-500/50 transition-all"
            >
              {sample.name}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
            ...
            <label className="border-2 border-dashed border-slate-700 hover:border-gold-500/50 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-colors bg-slate-950/50">
            ...
            {fileName && (
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-xs">
                ...
                <span className="text-gold-400 text-[11px] uppercase">{fileType}</span>
              </div>
            )}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-3 rounded-xl bg-slate-950 border border-slate-800 focus:border-gold-500 text-xs font-mono text-slate-200 outline-none resize-y"
              />
            </div>
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="p-5 sm:p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="px-2.5 py-1 rounded-md text-[11px] font-semibold uppercase tracking-wider bg-gold-500/15 text-gold-300 border border-gold-500/30">
                    {result.fileType}
                  </span>
                ...
                <h3 className="text-base font-bold text-white">Executive Plain-Language Summary</h3>
                  ...
                  {result.parties?.length ? (
                    <div className="flex flex-wrap gap-2">
                      {result.parties.map((p, idx) => (
                        <span
                          key={idx}
                          className="px-3 py-1 rounded-lg bg-slate-800 text-xs font-medium text-slate-200 border border-slate-700"
                        >
                          {p}
                        </span>
                      ))}
                ...
                <div className="p-5 sm:p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
                  ...
                  <div className="space-y-3">
                    {result.flaggedClauses.map((clause, idx) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-sm text-slate-100">{clause.clauseName}</span>
                          <span
                        ...
                        <div className="text-xs font-mono text-slate-400 italic bg-slate-900 p-2 rounded border border-slate-800">
                          &ldquo;{clause.textExcerpt}&rdquo;
                        </div>

                        <p className="text-xs text-slate-300 leading-relaxed pt-1">
                          <strong className="text-gold-400 font-semibold">Legal Impact: </strong>
                          {clause.explanation}
                        </p>
                      </div>
                    ))}
                ...
                {result.keyDates?.length ? (
                  <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-2">
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5 text-gold-400" /> Key Dates & Tenures
                    </span>
                    <ul className="space-y-2 text-xs">
                      {result.keyDates.map((d, idx) => (
                        <li key={idx} className="p-2 rounded bg-slate-950/60 border border-slate-800/80">
                          <div className="font-bold text-slate-200">{d.label}: <span className="text-gold-300 font-normal">{d.date}</span></div>
                          <div className="text-slate-400 mt-0.5">{d.implication}</div>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-2">
                    ...
                    <ul className="space-y-2 text-xs">
                      {result.financialTerms.map((f, idx) => (
                        <li key={idx} className="p-2 rounded bg-slate-950/60 border border-slate-800/80">
                          <div className="font-bold text-slate-200">{f.item}: <span className="text-emerald-300 font-normal">{f.amount}</span></div>
                          <div className="text-slate-400 mt-0.5">{f.condition}</div>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="p-4 rounded-xl bg-gradient-to-r from-navy-900 to-slate-900 border border-gold-500/40 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  <h4 className="text-sm font-bold text-white">Discovered an unfair clause or dispute?</h4>
                  <p className="text-xs text-slate-400">Consult with Nyay Mitra AI or generate a legal notice to safeguard your rights.</p>
                </div>
                <Link
                  href="/chat"
