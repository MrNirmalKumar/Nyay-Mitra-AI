...
export const LegalDisclaimer: React.FC<LegalDisclaimerProps> = ({ compact = false, className = "" }) => {
  if (compact) {
    return (
      <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-xs text-amber-200/90 ${className}`}>
  ...
  return (
    <div className={`p-4 rounded-xl bg-slate-900/80 border border-slate-800 text-slate-300 shadow-subtle ${className}`}>
